...
/// Registers the Bitwarden desktop app as a Windows passkey plugin authenticator.
pub fn register() -> std::result::Result<(), String> {
    tracing::debug!("Registering Windows passkey plugin authenticator");

    WebAuthnPlugin::initialize()
        .map_err(|err| format!("Failed to initialize Windows WebAuthn plugin COM server: {err}"))?;

    let clsid = CLSID
        .try_into()
        .map_err(|err| format!("Invalid authenticator CLSID `{CLSID}`: {err}"))?;
    ...
    plugin
        ...
        .map_err(|err| format!("Failed to register Windows WebAuthn plugin COM server: {err}"))?;
    ...
    WebAuthnPlugin::add_authenticator(&options)
        .map_err(|err| format!("Failed to add Windows WebAuthn plugin authenticator: {err}"))?;
    tracing::debug!("Windows passkey plugin authenticator registered");
...
impl BitwardenPluginAuthenticator {
    fn get_client(&self) -> Arc<WindowsProviderClient> {
        ...
        match client.as_ref() {
            ...
            None => {
                let new_client = Arc::new(WindowsProviderClient::connect());
...
impl PluginAuthenticator for BitwardenPluginAuthenticator {
    fn make_credential(
        ...
        self.callbacks
            .lock()
            .map_err(|_| "Cancellation callback storage is poisoned".to_string())?
