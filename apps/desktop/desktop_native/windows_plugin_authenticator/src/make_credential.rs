...
pub fn make_credential(
    ...
    let excluded_credentials: Vec<Vec<u8>> = request
        .exclude_credentials()
        .filter_map(|cred| cred.credential_id().map(<[u8]>::to_vec))
        .collect();
    ...
    tracing::debug!("Registration response received");
    ...
    tracing::debug!("Successfully created WebAuthn make credential response");
...
fn send_registration_request(
    ...
    tracing::debug!(
        user_handle_len = request.user_handle.len(),
        client_data_hash_len = request.client_data_hash.len(),
        supported_algorithm_count = request.supported_algorithms.len(),
        excluded_credential_count = request.excluded_credentials.len(),
...
fn create_make_credential_response(
    ...
    let webauthn_att_obj: HashMap<&str, &Value> = att_obj_items
        .iter()
        .filter_map(|(k, v)| k.as_text().map(|key| (key, v)))
        .collect();
...
mod tests {
    ...
    fn test_encode_make_credential_custom() {
        ...
        let ctap_att_obj = match create_make_credential_response(webauthn_att_obj) {
            Ok(value) => value,
            Err(err) => panic!("failed to encode make credential response: {err}"),
        };
