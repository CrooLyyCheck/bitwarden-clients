...
pub fn get_assertion(
    ...
    tracing::debug!(
        allowed_credential_count = allowed_credential_ids.len(),
        "Prepared get assertion request"
    );
    ...
    tracing::debug!("Assertion response received");
...
fn send_assertion_request(
    ...
    tracing::debug!(
        ...
        allowed_credential_count = request.allowed_credentials.len(),
        "Sending assertion request"
    );
...
mod tests {
    ...
    fn test_create_native_assertion_response() {
        ...
        let cbor = match create_get_assertion_response(
            credential_id,
            authenticator_data,
            signature,
            user_handle,
            ...
            Ok(value) => value,
            Err(err) => panic!("failed to encode assertion response: {err}"),
        };
