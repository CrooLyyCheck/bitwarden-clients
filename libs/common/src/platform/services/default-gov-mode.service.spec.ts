...
describe("DefaultGovModeService", () => {
  ...
  describe("globalIsGovMode", () => {
    it("returns true for Gov", async () => {
      const envService = createMockEnvironmentService(Region.Gov);
      const sut = new DefaultGovModeService(envService as any);

      expect(await sut.globalIsGovMode()).toBe(true);
    });

    it.each([Region.US, Region.EU, Region.SelfHosted])("returns false for %s", async (region) => {
      const envService = createMockEnvironmentService(region);
      const sut = new DefaultGovModeService(envService as any);

      expect(await sut.globalIsGovMode()).toBe(false);
    });
  });
  ...
  describe("isGovMode", () => {
    it("returns true for Gov", async () => {
      ...
      expect(await sut.isGovMode(mockUserId)).toBe(true);
    });
    ...
    it.each([Region.US, Region.EU, Region.SelfHosted])("returns false for %s", async (region) => {
      ...
      expect(await sut.isGovMode(mockUserId)).toBe(false);
    });
  });
});
